...
from stdm.data import STDMEntity, STDMDb, TableMapper,LookupTable
from foreign_key_mapper import ForeignRelationMapper,ForeignKey,LookupEntity
from sqlalchemy.exc import SQLAlchemyError

class STDMForm(BoundDialog):
    def __init__(self,parent,columns,Session=None, row=None, row_id=None, flush=True):
        BoundDialog.__init__(self, parent)
        self.session=Session
        self.cols=columns
        table=self.classFromTable(name)
        self.setWindowTitle("Editor")
        self.setDataReader(self.session,STDMEntity,'id')
        vbox=QVBoxLayout()
        self.setLayout(vbox)
        grid=LayoutLayout(vbox,QFormLayout())
        try:
            STDMEntity.household=ForeignKeyReferral(str,"household",'household',LookupTable,'id')
        except:
            pass
        
        self.mm=self.mapClass(STDMEntity)
...
class EntityHelper(DomainEntity):
    def session_entity(self, id):
        session = self.Session
        a = session.query(STDMEntity).filter(self.key_column==id.id).one()
        return session, a
...
class STDMEntityForm(EntityHelper):
    def __init__(self,table,cols,parent):
        ...
        self.key_column = STDMEntity.id
    
        self.table_cls = STDMEntity
        self.key_column = self.key_column
        self.list_display_columns = self.cols
        self.list_search_columns = self.cols

    def view_row(self,session,row):
        form = STDMForm(self.parent,self.list_display_columns,Session=self.Session,row=row)
        form.show() 
        form.exec_()
        session.close()
    ...
    def view(self, row):
        try:
            session, aa = self.session_entity(row)
    ...
    def delete(self, row):
        try:
            #session = self.Session()
            #r=session.query(STDMEntity).filter(STDMEntity.id==row.id).one()
            self.Session, aa = self.session_entity(row)
