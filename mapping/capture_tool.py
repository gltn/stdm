...
class StdmMapToolCapture(StdmMapToolEdit):
    ...
    '''
    def __init__(self,iface,mode = CAPTURE_NONE):
        self.iface = iface
        self.canvas = self.iface.mapCanvas()
        StdmMapToolEdit.__init__(self, self.iface)
        
        #Connect signals
        self.iface.legendInterface().currentLayerChanged.connect(self.onCurrentLayerChanged)
    ...
    def createVertexMarker(self):
        '''
        Function creates vertex marker that is used for marking new point on map.
        '''
        # get qgis settings
        settings = QSettings()
    ...
    def canvasMoveEvent(self,e):
        ...
        if self._mode != CAPTURE_POINT and self._tempRubberBand != None and self._capturing:
    ...
    def nextPoint(self,point):
        ...
        if not isinstance(currLayer,QgsVectorLayer):
            QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","No Vector Layer Found"), \
        ...
        try:
            ...
            QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","Transformation to map coordinate failed"), \
                                     QgsMessageLog.CRITICAL)
        ...
        try:
            ...
            QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","Transformation to layer coordinate failed"), \
                                     QgsMessageLog.CRITICAL)
            return 2,layerPoint,mapPoint
        
        snapResults = []
        result,snapResults = self._snapper.snapToBackgroundLayers(point,snapResults)
    ...
    def addVertex(self,point,isLayerPoint = False):
        ...
        return 0 in case of success, 1 if current layer is not a vector layer, 2 if layer coordinate transformation failed,
        3 if map coordinate transformation failed.
        '''
        layerPoint = None
        mapPoint = None
        
        if self.captureMode() == CAPTURE_NONE:
            QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","Invalid capture mode"), \
                                     QgsMessageLog.CRITICAL)
            return 2
        ...
        if not isLayerPoint:
            ...
            if result != 0: 
                QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","Next point failed"), \
                                         QgsMessageLog.CRITICAL)
                return result
        else:
            layerPoint = point
            ...
            try:
                mapPoint = self.toMapCoordinates(self.currentVectorLayer(),layerPoint)
            except QgsCsException:
                QgsMessageLog.logMessage(QApplication.translate("StdmMapToolCapture","Transformation to map coordinate failed"), \
                                     QgsMessageLog.CRITICAL)
                return 3,layerPoint,mapPoint
        
        geomType = QGis.Polygon if self.captureMode() == CAPTURE_POLYGON else QGis.Line
        
        if self._rubberBand == None:
    ...
    def undo(self):
        ...
        '''
        if self._rubberBand != None:
            if len(self._captureList) == 0 or self._rubberBand.numberOfVertices() == 0:
                return
            ...
            if self._rubberBand.numberOfVertices() > 1:
                if self._tempRubberBand.numberOfVertices() > 1:
                    ...
                    point = self._rubberBand.getPoint(0,self._rubberBand.numberOfVertices() - 1)
                    self._tempRubberBand.movePoint(self._tempRubberBand.numberOfVertices() - 2,point)
                    
            else:
                self._tempRubberBand.reset(True if self.captureMode() == CAPTURE_POLYGON else False)
    ...
    def closePolygon(self):
        '''
        Util function for closing polygon by linking the last point to the first one.
        '''
