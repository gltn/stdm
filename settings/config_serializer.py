...
class AutoGeneratedColumnSerializer(ColumnSerializerCollection):
    ...
    def _obj_args(cls, args, kwargs, element, assoc_elements,
        ...
        if not code_ele.isNull():
            ...
            char_lengths = code_ele.attribute(
                AutoGeneratedColumnSerializer.CHAR_LENGTHS,
                ''
            )
            pad_dir = code_ele.attribute(
                AutoGeneratedColumnSerializer.PAD_DIR,
                ''
            )
            ...
            if not char_lengths:
                ...
                char_lengths = char_lengths.split(',')

            if not pad_dir:
                pad_dir = []
            else:
                pad_dir = pad_dir.split(',')
            ...
            kwargs['char_lengths'] = char_lengths
            kwargs['pad_dir'] = pad_dir
    ...
    def _write_xml(cls, column, column_element, document):
        ...
        dt_element.setAttribute(
            'charLengths', ','.join(str(n) for n in column.char_lengths)
        )
        dt_element.setAttribute(
                'padDir', ','.join(column.pad_dir)
        )
