...
class TestShellCommands(Shell):
    ...
    def test_help_commands(self):
        ...
        # we can only tests that we get some output
...
class TestShellRepository(Shell):
    ...
    def test_source(self):
        ...
        # We can also send the source to a file... tests that too
...
class TestShellDatabase(Shell, DB):
    ...
    def _run_test_sqlfile(self, upgrade_script, downgrade_script):
        # TODO: add tests script that checks if db really changed
        repos_path = self.tmp()
        repos_name = 'repos'
    ...
    def test_command_test(self):
        ...
        result = self.env.run('migrate tests %s %s' % (self.url, repos_path))
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
        ...
        result = self.env.run('migrate tests %s %s bla' % (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
    ...
    def test_rundiffs_in_shell(self):
        ...
        self.meta.reflect()
        self.meta.drop_all()  # in case junk tables are lying around in the tests database
