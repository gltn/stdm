...
class AutoGeneratedLineEdit(ForeignKeyLineEdit):
    ...
    def set_code_from_code_column(self):
        ...
        parent_form = self._entity_dialog
    ...
    def set_code_from_columns(self):
        ...
        for i, column_name in enumerate(self.column.columns):
            ...
            try:
                ...
                id_value = parent_form.entity_editor.attribute_mapper(column_name).valueHandler().value()

            if column.TYPE_INFO in ['LOOKUP', 'ADMIN_SPATIAL_UNIT', 'FOREIGN_KEY']:
                id_value = entity_id_to_attr(
            ...
            char_length = 0
            if len(self.column.char_lengths) > 0:
                char_length = int(self.column.char_lengths[i])

            id_value_str =  str(id_value)
            id_value_len = len(id_value_str)

            if char_length > 0 and id_value_len < char_length:
                pad = char_length - id_value_len

                pad_dir = 'PAD_LEFT'
                if len(self.column.pad_dir) > 0:
                    pad_dir = self.column.pad_dir[i]

                if pad_dir == 'PAD_LEFT':
                    id_value_str = ('0'*pad) + id_value_str
                else:
                    id_value_str = id_value_str + ('0'*pad)

            prefix_code.append(u'{}{}'.format(id_value_str, separator))
