...
from stdm.utils.util import entity_id_to_attr
...
class AutoGeneratedLineEdit(ForeignKeyLineEdit):
    ...
    def set_code_from_columns(self):
        ...
        entity = parent_form._entity
        # model = entity_model(entity)
        prefix_code = []
        for column_name in self.column.columns:
            column = parent_form._entity.columns[column_name]

            id_value = parent_form.attribute_mapper(column_name).valueHandler().value()
            if column.TYPE_INFO == ['LOOKUP', 'ADMIN_SPATIAL_UNIT', 'FOREIGN_KEY']:
                code = entity_id_to_attr(entity, 'code', id_value)
                prefix_code.append(unicode(code))
            else:
                prefix_code.append(unicode(id_value))

        self.current_item = ''

        # The prefix and separator should be '' for a serial.
        self._code = self.code_generator.generate(
            self.column.separator.join(prefix_code),
            self.column.separator,
            self.column.leading_zero
        )
