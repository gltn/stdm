...
class AutoGeneratedLineEdit(ForeignKeyLineEdit):
    ...
    def __init__(self, *args, **kwargs):
        #Use a different pixmap
        px = QPixmap(':/plugins/stdm/images/icons/code.jpg')
        kwargs['pixmap'] = px
        self._code = None
    ...
    def set_code_from_lookup(self):
        ...
        if result == QDialog.Accepted:
            ...
            self._code = self.generate_unique_code(
    ...
    def generate_unique_code(self, prefix, separator, leading_zero):
        """
        ...
        if len(record_match) == 0:
            ...
            if prefix == '':
                ...
                # if the separator is empty - '', split by numbers and letters.
                    ...
                    match = re.match(r'([a-zA-Z]+)([0-9]+)', record_match[0], re.I)
                    if match:
